...
**2. Kill that process (replace `<PID>` with the actual number):**
```bash
taskkill /PID <PID> /F
```
...
Enable 2-Step Verification on your Google account, then generate an App Password under **Security → App Passwords**.
